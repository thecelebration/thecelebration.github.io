(function () {
    ...
    widget.init = function () {
        ...
        function InsertWidget(element, data){
            ...
            if(data.shared_id && data.shared_id != ""){
                full_url += "shared_id=" + encodeURIComponent(data.shared_id) + "&";
            }

            if(data.custom_aff_tracking && data.custom_aff_tracking != ""){
                full_url += "custom_aff_tracking=" + encodeURIComponent(decodeURIComponent(data.custom_aff_tracking)) + "&";
            }

            full_url += "url=" + encodeURIComponent(data.url) + "&title=" + encodeURIComponent(document.title); // + "&sid=" + sid;

            var referrer_search_term = element.getAttribute("data-query");

            if(referrer_search_term){
                full_url += "&referrer_term=" + encodeURIComponent(referrer_search_term);
            }
            ...
            widget.socket = new widget.easyXDM.Socket({
                ...
                props: props,
                onLoad: function() {
                    //We don't use this stats so we have disable it.
    ...
    widget.CreateTrackingPixel = function(element, data){
        ...
        data.sid = sid;
        //We have disabled this stats since we don't use it for now.
...
//we had an idea to make a heat map we don't use it anymore
